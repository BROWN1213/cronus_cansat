...
CansatGPS_NMEA::CansatGPS_NMEA(GPS_State &_state) :
...
*/
bool CansatGPS_NMEA::decode(char c)
{
...
// internal utilities
//
int16_t CansatGPS_NMEA::_from_hex(char a)
{
    if (a >= 'A' && a <= 'F')
        return a - 'A' + 10;
    else if (a >= 'a' && a <= 'f')
...
int32_t CansatGPS_NMEA::_parse_decimal_100(const char *p)
{
    char *endptr = nullptr;
 ...
 */
uint32_t CansatGPS_NMEA::_parse_degrees()
{
    char *p, *q;
    uint8_t deg = 0, min = 0;
    float frac_min = 0;
